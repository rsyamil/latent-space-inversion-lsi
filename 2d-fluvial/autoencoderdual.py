...
def inspect_LSI(LSI, M_test, D_test):
    ...
    for case in cases:
        ...
        plt.plot(D_test_hat[case, :], c='r', label='Pred.', alpha=0.4)
        plt.ylim([0, 1])
        plt.title('Data')
        plt.legend()
        ...
        plt.imshow(M_test_hat_reg[1][case, :, :, 0], cmap=plt.cm.GnBu, vmin=0, vmax=1)
        plt.title('Inversion')
        plt.grid(False), plt.xticks([]), plt.yticks([])
        
        #predicted model (i.e. inversion) colormap limit removed to show diffr
        ax = f.add_subplot(1, 4, 4)
        plt.imshow(M_test_hat_reg[1][case, :, :, 0], cmap=plt.cm.GnBu)
        plt.title('Inversion')
        plt.grid(False), plt.xticks([]), plt.yticks([])
        f.savefig('readme/test_sigs_ref_invs_'+str(case)+'.png')


def inspect_LSI_z(LSI, M_test, D_test):
